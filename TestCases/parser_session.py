...
from utilities.User_PayLoad import *
...
response_logsUser=requests.get(url+"/login",params={"username":"pavan2","password":"pavan2"},headers=headers,)
assert response_logsUser.status_code==200
print(response_logsUser.json())

#Get Logs user into the system(Authentication)
se=requests.session()
se.auth=auth=('pavan2',getPassword())
...
#Get user detail
response_get=requests.get(url+"/pavan",)
#assert response_get.status_code==200
print(response_get.json())

#Delete user
response_delete=requests.delete(url+"/pavan3",headers=headers,)
assert response_delete.status_code==200
print(response_delete.json())

#modify user
response_modify=hed.put(url+"/pavan",json=addmodifiedUser(),headers=headers)
